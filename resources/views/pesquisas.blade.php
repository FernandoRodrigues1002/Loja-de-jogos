...
<body>
    ...
    @if($pesquisas->isNotEmpty())
    <div class="container" style="margin-top: 100px; margin-bottom: 100px;">
        <div class="card__title">
            <h2 style="text-transform: uppercase; text-align: left;">Resultados para: ' {{ $query }} '</h2>
        </div>
        <div class="card__content card__content__1">
            <div class="row">
                @foreach ($pesquisas as $pesquisa)
                <div class="col-md-3 col-sm-6 mb-4" style="margin-top: 50px;"> <!-- Adiciona margem horizontal e espaçamento entre os itens -->
                    <article class="card__article">
                        <div class="card__image">
                            @if ($pesquisa->Fotos->count() > 0)
                            <img src="{{$pesquisa->Fotos[0]->IMAGEM_URL}}" alt="image" class="card__img">
                            @endif
                        </div>

                        <div class="card__data">
                            <h3 class="card__name">{{ $pesquisa->PRODUTO_NOME }}</h3>
                            <p class="card__description"> Por apenas: </p>
                            <p class="card__description card__preco">
                                @if ($pesquisa->PRODUTO_PRECO == 0.00)
                                    Gratuito
                                @else
                                    R$ {{ $pesquisa->PRODUTO_PRECO }}
                                @endif
                            </p>
                                
                            <a href="{{ route('produto.show', $pesquisa->PRODUTO_ID) }}" class="card__button">Veja mais</a>
                        </div>
                    </article>
                </div>
                @endforeach
    ...
    <div class="card__title">
        <h2 style="text-transform: uppercase; text-align: left; margin-top:300px; margin-bottom:300px;">Nenhuma resultado para: ' {{ $query }} '</h2>
    </div>
